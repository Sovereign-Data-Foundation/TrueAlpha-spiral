...
def _check_command(cmd_tokens):
    ...
    while cmd_idx < len(cmd_tokens) and env_re.match(cmd_tokens[cmd_idx]):
        cmd_idx += 1
    ...
    if cmd_name == 'git':
        ...
        for i in range(cmd_idx + 1, len(cmd_tokens)):
            token = cmd_tokens[i]
            if (token.startswith("--ext-cmd") or
                ...
                return False, f"Unauthorized execution option '{token}' for git"
