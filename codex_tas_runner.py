...
def _check_command(cmd_tokens):
    ...
    if cmd_name in ('bash', 'python', 'python3'):
        for arg in cmd_tokens[cmd_idx+1:]:
    ...
    if cmd_name == 'git':
        ...
        dangerous_globals = ('--ext-cmd', '--exec-path', '--config', '--config-env', '--paginate')
        subcommand_seen = None
        for arg in cmd_tokens[cmd_idx+1:]:
            if not arg.startswith('-') and subcommand_seen is None:
                subcommand_seen = arg
                if subcommand_seen == 'config':
                     return False, "Unauthorized git command: config"
                continue

            for dang in dangerous_globals:
                if arg == dang or arg.startswith(dang + '='):
                    return False, f"Unauthorized execution option '{arg}' for git"

            if arg.startswith('-') and not arg.startswith('--'):
                if 'c' in arg:
                    if subcommand_seen not in safe_subcommands:
                        return False, f"Unauthorized execution option '{arg}' for git"
