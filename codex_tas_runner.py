...
def _check_command(cmd_tokens):
    ...
    if cmd_name in ('bash', 'python', 'python3'):
        for token_arg in cmd_tokens[idx+1:]:
            if token_arg in ('-c', '-m') or token_arg.startswith('-c=') or token_arg.startswith('-m=') or (token_arg.startswith('-c') and len(token_arg) > 2) or (token_arg.startswith('-m') and len(token_arg) > 2):
    ...
    if cmd_name == 'git':
        ...
        while i < len(cmd_tokens):
            ...
            if cmd_tokens[i] in ('-C', '-c', '--work-tree', '--git-dir', '--namespace'):
                ...
                i += 1
        subcommand = cmd_tokens[subcommand_idx].lower() if subcommand_idx != -1 else None
        ...
        for i in range(idx + 1, len(cmd_tokens)):
            ...
            if token == '-c' or token.startswith('-c=') or (token.startswith('-c') and len(token) > 2):
                # -c is only allowed after the subcommand for certain subcommands
