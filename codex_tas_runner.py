...
def _check_command(cmd_tokens):
    ...
    idx = 0
    while idx < len(cmd_tokens) and re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*=', cmd_tokens[idx]):
        idx += 1
    ...
    if cmd_name == 'git':
        for arg in cmd_tokens[idx+1:]:
            ...
            if (arg.startswith('-c') or arg.startswith('--ext-cmd') or
                arg.startswith('--exec-path') or arg.startswith('--config') or
                arg.startswith('--config-env') or arg.startswith('--paginate')):
                return False, f"Unauthorized global option '{arg}' for git"
