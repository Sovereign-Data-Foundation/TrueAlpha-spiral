...
## 2024-05-18 - [Fix Git Command Injection Filter Bypass]
...
## $(date +%Y-%m-%d) - Fix command injection bypass in codex_tas_runner.py
**Vulnerability:** Command injection and unsafe git options execution bypass where an attacker could prepend multiple environment variable assignments (e.g., `A=1 B=2 bash -c ...`) to trick the validation logic into ignoring dangerous flags like `-c`, or passing global parameters modifying process execution (e.g. `core.pager`).
**Learning:** The existing manual token parsing logic only assumed a single environment variable could prepend a command and therefore failed to correctly isolate the root command if multiple env vars were supplied. It was also insufficiently strict regarding the placement of dangerous global Git options (like `-c` and `--config`), treating them permissively before a subcommand.
**Prevention:** Always iterate through all tokens sequentially to consume prefix definitions (like environment variables) before evaluating the command syntax against an allowlist. For complex extensible tools like Git, strictly define safe subcommands and enforce that any global options manipulating behavior (like `-c`) only appear if explicitly tied to a safe operation. Use regex instead of naive string checks to accurately identify pattern boundaries (e.g. `re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*=', token)`).
