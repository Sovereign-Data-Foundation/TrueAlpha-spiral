...
## 2024-05-18 - [Preventing Command Injection in Allowlisted Tools]
**Vulnerability:** Allowlisting `git`, `python`, and `bash` is not enough to prevent command execution because arguments such as `-c`, `--config` or `--ext-cmd` can be injected to run unauthorized commands. In Python `-c` allows inline code execution, in `git` `--config=core.pager=!sh` allows shell execution.
**Learning:** Argument filtering must be robust against `=` assignment syntax (`-c=...`) and contiguous flags (`-xc`). Naive `if arg == "-c":` checks are bypassable.
**Prevention:** Use `.startswith()` checks. In `git`, explicitly differentiate between dangerous global options and safe subcommands (`-c` is safe after `checkout` or `commit` but not globally). Use regex to skip over initial environment variables correctly before validating command names and arguments.
