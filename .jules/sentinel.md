...
## $(date +%Y-%m-%d) - Prevent command injection via environment variables and malicious global options in codex_tas_runner.py
**Vulnerability:** Command injection was possible in `codex_tas_runner.py` by prepending environment variables to unauthorized commands (e.g., `VAR=1 bash -c "malicious"`), which bypassed the `shlex.split` command validation checks. Also, `git` global options (like `-c` and `--config`) could be used maliciously before the subcommand.
**Learning:** `shlex.split` treats environment variable assignments as separate tokens, which can shift the command name to a subsequent index. Exact matches for execution options (like `-c`) can be bypassed by combined options (like `-xc` or `-c=`).
**Prevention:** Iterated past tokens matching the regex `^[a-zA-Z_][a-zA-Z0-9_]*=` to find the actual command name. Used `.startswith('-') and not .startswith('--')` checks to capture execution options for interpreters (`bash`, `python`) and tracked if a `git` subcommand had been seen yet to distinguish global options from safe subcommand arguments.
