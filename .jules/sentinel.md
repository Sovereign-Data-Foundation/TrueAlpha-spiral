...
## 2024-05-24 - Fix Command Injection Bypasses in codex_tas_runner.py
**Vulnerability:** The existing command validation loop in `codex_tas_runner.py` had several command injection bypasses. For example, it failed to parse commands properly if multiple environment variables were prepended (e.g., `A=1 B=2 cmd`). Also, it allowed command injection bypasses via `git` global config options like `--config-env` and combined execution flags for bash/python like `-xc`.
**Learning:** Checking for substrings like `'c' in token_arg` for flags starting with `-` is necessary to prevent bypasses using combined execution flags in bash/python (e.g., `-xc`). We also must check all `git` arguments for dangerous global options (`-c`, `--ext-cmd`, `--exec-path`, `--config`, `--config-env`, `--paginate`), unless the `-c` is a safe subcommand argument.
**Prevention:** Always use regex matching to robustly skip environment variable assignments and isolate the actual command. Explicitly block combined execution flags and validate all global configuration options in extensible commands like `git`.
