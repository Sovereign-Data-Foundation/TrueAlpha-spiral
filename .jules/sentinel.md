...
## 2026-09-09 - Command Injection in Script Validation
**Vulnerability:** Shell script validation in `codex_tas_runner.py` improperly handled multiple environment variables preceding a command, and failed to sanitize Git arguments, allowing command injection via Git global options (e.g., `git -c core.pager="!sh -c 'hacked'"`) and bypassed bash/python execution blocks (e.g., `bash -c=`).
**Learning:** Checking for command bypasses requires correctly parsing the POSIX shell structure for variable assignments at the beginning of commands to accurately identify the root command. Additionally, tools like Git extend execution boundaries and must have global configuration flags actively blocked.
**Prevention:** Implement strict prefix-matching validation logic that securely skips any number of matched environment variables using `re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*=')` and uses length/startswith checks (e.g. `startswith('-c=')`) to properly detect options and block unsafe Git subcommands or arguments.
