...
## 2026-04-03 - [Command Injection via Concatenated Git Global Options]
**Vulnerability:** Arbitrary command execution in `tas_pythonetics/src/tas_pythonetics/git_safety.py`. The `GitActionGuard.authorize_command` method checked for dangerous global options using exact string matches on flags like `"-c"`. This allowed bypasses using concatenated options, such as `git -ccore.pager=calc`, which Git correctly interprets but the naive exact match filter missed.
**Learning:** Command-line parsers like `shlex` output concatenated options (e.g., `-ccore.pager=calc`) as a single string token. Filtering logic must check if the token `startswith` the dangerous flag (e.g., `"-c"`), rather than using an exact equality check (e.g., `== "-c"`), because GNU-style short options often permit concatenation with their values.
**Prevention:** When validating arguments for wrapped command-line tools, always check for prefixes using `.startswith()` when the option is a short flag (like `-c`) or long flag with `=` (like `--config-env=`), rather than exact equality, to account for concatenated values.
