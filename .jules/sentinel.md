...
## 2024-10-24 - [Command Injection bypass via Multiple Environment Variables]
**Vulnerability:** A command injection bypass was found in `codex_tas_runner.py`. The previous checks securely blocked dangerous commands and arguments, but failed to skip multiple environment variables appended at the beginning of a command string. For example, `FOO=1 BAR=2 bash -c 'echo 1'` allowed `-c` because `BAR=2` was wrongly parsed as the executable command, shifting the argument indexes and preventing `-c` from being flagged for `bash`.
**Learning:** Checking for environment variable assignments in tokenized shell commands is insufficient if it only assumes a single assignment per token or line. Real-world commands may prefix execution with multiple variable assignments, which shifts the actual command index and breaks simple positional token filtering.
**Prevention:** Iteratively parse and skip *all* matching token prefixes (e.g. using `re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*=')`) at the start of a command before extracting the base executable command name to ensure execution flags are evaluated against the correct executable.
