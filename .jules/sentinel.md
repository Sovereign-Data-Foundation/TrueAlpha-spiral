...
## 2023-10-25 - [Command Injection via Git Global Options]
**Vulnerability:** Extensible commands like `git` were permitted by the allowlist but were not properly validated against global configuration override options (like `-c`, `--exec-path`, `--config`, etc.). This allowed command injection through git's configuration mechanisms (e.g., overriding `core.pager`).
**Learning:** Even when core runtime options (like `bash -c`) are secured, seemingly safe CLI tools often provide their own command execution mechanisms via global flags.
**Prevention:** Always maintain a strict allowlist for both primary commands *and* their potential execution-modifying global options. For `git`, explicitly reject configuration modifiers and securely differentiate between safe subcommand arguments (e.g., `git switch -c`) and global arguments.
