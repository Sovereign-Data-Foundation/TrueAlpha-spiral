...
## 2024-11-20 - [Fix command injection bypass via environment variables syntax]
**Vulnerability:** Arbitrary command execution in `codex_tas_runner.py`. The `validate_script` function failed to properly parse commands with leading environment variables or arguments containing `=` (e.g. `./malicious=script`). The naive string split resulted in command validation checks being bypassed or performed on the wrong target arguments.
**Learning:** Shell scripts allow setting environment variables prior to command execution within the same line, parsed as separate tokens by shlex. If validation logic naively seeks out `=` signs to skip environment variables, an attacker can bypass execution rules using paths or binaries with `=` in their name, or bypass arguments if multiple environment variables are provided and the scanner only skips one token.
**Prevention:** Explicitly use regular expressions (e.g. `^[a-zA-Z_][a-zA-Z0-9_]*=`) to validate that tokens prior to the main command are strictly valid environment variable assignments, and iterate through all of them until reaching the true executable token.
