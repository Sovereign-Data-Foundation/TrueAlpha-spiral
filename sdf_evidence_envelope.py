...
class EvidenceVerdict:
    ...
    ``receipt_hash``     — deterministic SHA-256 of the verdict body; serves as
                           the non-repudiable receipt identifier regardless of
...
def verify_evidence(
    ...
    *,
    ...
    # --- 2. Lineage intact --------------------------------------------------
    if failed is None:
        results["lineage_intact"] = _check_lineage(envelope)
...
def _check_lineage(envelope: SDFEvidenceEnvelope) -> bool:
    """Return True iff lineage fields are well-formed.
    ...
    * The ``canonical_hash`` stored in the envelope must equal the recomputed
    ...
    return envelope.canonical_hash == expected
